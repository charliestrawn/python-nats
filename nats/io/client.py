...
from nats.io.errors import ErrAuthorization
from nats.io.errors import ErrConnectionClosed
from nats.io.errors import ErrMaxPayload
from nats.io.errors import ErrNoServers
from nats.io.errors import ErrServerConnect
from nats.io.errors import ErrSlowConsumer
from nats.io.errors import ErrStaleConnection
from nats.io.errors import NatsError
from nats.io.utils import new_inbox
from nats.protocol.parser import MAX_CONTROL_LINE_SIZE
...
class Client(object):
    ...
    def __init__(self):
        ...
        # Storage and monotonically increasing index for subscription
        # callbacks.
        self._subs = {}
        self._ssid = 0
    ...
    def connect(self,
        ...
        yield nc.connect({
            'servers': [
                ...
                'nats://192.168.2.10:4222'
            ]
        })

        # User and pass are to be passed on the uri to authenticate.
        yield nc.connect({
            'servers': ['nats://hello:world@192.168.1.10:4222']
        })
        ...
        while True:
            try:
                ...
                # Check when was the last back off before reconnecting
                if s.last_attempt is not None:
                    now = time.time()
                    if (now - s.last_attempt) < self.options["reconnect_time_wait"]:  # noqa e501
                        yield tornado.gen.sleep(self.options["reconnect_time_wait"])  # noqa e501
    ...
    def connect_command(self):
        ...
        if "auth_required" in self._server_info:
            if self._server_info["auth_required"] is True:
    ...
    def subscribe(self,
                  ...
                  is_async=False):
        """
        Sends a SUB command to the server. Takes a queue parameter which can
        be used in case of distributed queues or left empty if it is not the
        case, and a callback that will be dispatched message for processing
        them.
        """
        if self.is_closed:
            raise ErrConnectionClosed
    ...
    def _subscribe(self, sub, ssid):
        """
        Generates a SUB command given a Subscription and the subject sequence
        id.
        """
        sub_cmd = b''.join([
            SUB_OP,
    ...
    def auto_unsubscribe(self, sid, limit=1):
        """
        Sends an UNSUB command to the server.  Unsubscribe is one of the
        basic building blocks in order to be able to define request/response
        semantics via pub/sub by announcing the server limited interest a
        priori.
        """
        b_limit = b''
        if limit > 0:
    ...
    def _process_ping(self):
        ...
        does not reply a PONG back a number of times, it will close the
        connection sending an `-ERR 'Stale Connection'` error.
        """
    ...
    def _process_pong(self):
        ...
        to the server as a failure detector to close connections to unhealthy
        servers. For each PING the client sends, we will add a respective PONG
        future.
        """
    ...
    def _process_msg(self, sid, subject, reply, data):
        ...
        if sub.cb is not None:
            if sub.is_async:
                ...
                if maybe_future is not None and type(maybe_future) is tornado.concurrent.Future:  # noqa e501
                    yield maybe_future
    ...
    def _process_connect_init(self):
        ...
        next_op = yield self.io.read_until(_CRLF_, max_bytes=MAX_CONTROL_LINE_SIZE)  # noqa e501
    ...
    def _next_server(self):
        ...
        for server in self._server_pool:
            if self.options["max_reconnect_attempts"] > 0 and (server.reconnects > self.options["max_reconnect_attempts"]):  # noqa e501
                continue
            else:
                s = server
            return s
    ...
    def _schedule_primary_and_connect(self):
        ...
        while True:
            ...
            try:
                ...
                # Continue trying to connect until there is an available
                # server or bail in case there are no more available
                # servers.
                self._status = Client.RECONNECTING
                continue
    ...
    def _process_err(self, err=None):
        """
        Stores the last received error from the server and dispatches
        the error callback.
        """
    ...
    def _flusher_loop(self):
        ...
        while True:
            ...
            try:
                ...
                if not self.is_connected or self.is_connecting or self.io.closed():  # noqa e501
                    break
